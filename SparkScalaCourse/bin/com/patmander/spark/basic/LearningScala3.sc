object LearningScala3 {
 ...
 def upperIt(x: String): String = {
 	x.toUpperCase()
 }                                                //> upperIt: (x: String)String
 
 def lowerIt(x: String): String = {
 	x.toLowerCase()
 }                                                //> lowerIt: (x: String)String
  
 println(upperIt("boom"))                         //> BOOM
 
 def transformString(x:String, f: String => String): String = {
 	f(x)
 }                                                //> transformString: (x: String, f: String => String)String
 
 transformString("Foo", upperIt)                  //> res3: String = FOO
 
 transformString("Foo", lowerIt)                  //> res4: String = foo
 
 transformString("Foo", x => {println(s"lowering: $x");lowerIt(x.toUpperCase())})
                                                  //> lowering: Foo
                                                  //| res5: String = foo
}